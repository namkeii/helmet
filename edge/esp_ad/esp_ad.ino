...
void loop()
{
    ...
    WiFiClient fog;
    for(int fog_part = 160; fog_part < 170; fog_part++){
      if (!fog.connect(host+string(fog_part), port)) { Serial.println("either host error or port error"); }
      else{
    ...
    delay(10000);
    for(int hel_part = 160; hel_part < 170; hel++){
      if (!fog.connect(host+string(hel_part), port)) { Serial.println("either host error or port error"); }
      else{
        Serial.println("Connected to the helmet");
